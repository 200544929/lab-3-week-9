...
function createHamburger(options) {
    // Define default hamburger structure
    ...
    hamburger.describe = function() {
      const garnishes = this.garnishes.join(', ');
      const cheeses = this.cheeses.join(', ');
      const sauces = this.sauces.join(', ');
      const toppings = this.toppings.join(', ');
  
      // Construct the description sentence
      return `This hamburger has a ${this.bunType} bun with a ${this.meatPatty} patty` +
             `${this.pattyCount > 1 ? ' x' + this.pattyCount : ''}, ` +
             `${garnishes ? garnishes + ', ' : ''}` +
             `${cheeses ? cheeses + ', ' : ''}` +
             `${sauces ? 'topped with ' + sauces : ''} ` +
             `${toppings ? 'and ' + toppings + ' on top.' : ''}`.trim();
    };
  ...
  const myHamburger = createHamburger({
    ...
    meatPatty: 'chicken',
    pattyCount: 2,
    toppings: ['pickles', 'hot peppers', 'olives']
  });
  
  console.log(myHamburger.describe());
  // Output: This hamburger has a brioche bun with a chicken patty x2, lettuce, tomato, cheddar, gouda, topped with mayo, mustard and pickles, hot peppers, olives on top.
